#!/usr/bin/env bash
# Beta ops console menus (sourced by Control Center or run via scripts/beta/diagnostic/beta-debug.sh).

set -euo pipefail

# shellcheck disable=SC2034
ops_console_beta_init() {
  if [[ -n "${OPS_ROOT_DIR:-}" ]]; then
...
header() {
  ...
  echo "  ╚██████╔╝██║  ██║██║  ██║    ██████╔╝███████╗   ██║   ██║  ██║"
  echo "   ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝   ╚═════╝ ╚══════╝   ╚═╝   ╚═╝  ╚═╝"
  echo -e "${NC}${BOLD}                   Ops Console :: BETA${NC}"
  echo -e "${CYAN}  ─────────────────────────────────────────────────────────────${NC}"
  echo ""
}
...
menu_connectivity() {
  while true; do
    ...
    echo "    5) VPN container → Desktop ping"
...
import redis, os
try:
    c = redis.Redis.from_url(os.environ.get('REDIS_URL', 'redis://redis:6379/0'))
    print('  ✓ PING:', c.ping())
    print('  ✓ QUEUE DEPTH:', c.llen('uah:parse_jobs'))
except Exception as e:
    print('  ✗ FAILED:', type(e).__name__, str(e))
"
        echo ""
        echo -e "${CYAN}  → Backend → Database${NC}"
        docker exec uah-beta-db psql -U uah -d uah_beta -c "SELECT current_database(), now();" 2>&1 | sed 's/^/  /'
        echo ""
...
"
      ...
      4)
        ...
        docker exec uah-beta-db psql -U uah -d uah_beta -c "
          SELECT current_database() as db, inet_server_addr() as host, now() as time;
        " 2>&1 | sed 's/^/  /'
        press_enter ;;
      5)
        header
  ...
  while true; do
    ...
    case $choice in
      1)
        ...
        docker exec uah-beta-redis redis-cli LLEN uah:parse_jobs | sed 's/^/  /'
        echo ""
      ...
      2)
        ...
        docker exec uah-beta-redis redis-cli FLUSHDB | sed 's/^/  /'
...
menu_logs() {
  while true; do
    ...
    case $choice in
      ...
      4)
        header
        echo -e "${BOLD}  Cloudflared Logs${NC}\n"
        $COMPOSE logs --tail=30 cloudflared 2>&1 | sed 's/^/  /'
        press_enter ;;
      5)
        header
...
menu_network() {
  while true; do
    ...
    echo "    5) Re-apply WireGuard routing rules"
    echo "    6) Re-apply cross-bridge iptables rules"
    echo "    7) Rollback all routing rules"
    echo "    8) Full network re-apply (5 + 6)"
    echo ""
    echo "    0) ← Back"
    echo ""
    read -rp "  Choice: " choice
    case $choice in
      ...
      5)
        header
        echo -e "${BOLD}  Re-apply WireGuard Routing${NC}\n"
        VPN_CONTAINER=uah-dev-vpn \
        BACKEND_CONTAINER=uah-beta-backend \
        NETWORK_NAME=uah-infra \
        SOURCE_CIDR=172.18.0.0/16 \
        bash "$SCRIPTS_BETA_DIR/network/apply_desktop_ollama_temp_route.sh" | sed 's/^/  /'
        press_enter ;;
      6)
        header
        echo -e "${BOLD}  Re-apply Cross-Bridge Rules${NC}\n"
        ...
        echo -e "  ${GREEN}✓ Cross-bridge rules applied: $BETA_BR <-> $INFRA_BR${NC}"
        press_enter ;;
      7)
        header
        echo -e "${BOLD}  Rollback All Routing Rules${NC}\n"
        bash "$SCRIPTS_BETA_DIR/network/rollback_desktop_ollama_temp_route.sh" | sed 's/^/  /'
        ...
        if [[ -n "$BETA_BRIDGE" && -n "$INFRA_BRIDGE" ]]; then
          ...
          echo -e "  ${GREEN}✓ Cross-bridge rules removed.${NC}"
        fi
        press_enter ;;
      8)
        header
        echo -e "${BOLD}  Full Network Re-apply${NC}\n"
        VPN_CONTAINER=uah-dev-vpn \
        BACKEND_CONTAINER=uah-beta-backend \
        NETWORK_NAME=uah-infra \
        SOURCE_CIDR=172.18.0.0/16 \
        ...
        sudo iptables -C DOCKER-USER -i "$INFRA_BR" -o "$BETA_BR" -j ACCEPT 2>/dev/null || \
          sudo iptables -I DOCKER-USER -i "$INFRA_BR" -o "$BETA_BR" -j ACCEPT
        echo -e "  ${GREEN}✓ All network rules applied.${NC}"
        press_enter ;;
      0) return ;;
    esac
  done
...
menu_database() {
  while true; do
    ...
    case $choice in
      ...
      5)
        header
        echo -e "${BOLD}  Raw SQL Query${NC}\n"
        echo -e "${YELLOW}  Enter SQL (single line, end with semicolon):${NC}"
        read -rp "  SQL> " sql_query
        if [[ -n "$sql_query" ]]; then
          docker exec uah-beta-db psql -U uah -d uah_beta -c "$sql_query" | sed 's/^/  /'
        fi
        press_enter ;;
      6)
        header
...
ops_beta_main_loop() {
  while true; do
    header
    # Quick status bar
    BACKEND_UP=$(docker ps --format '{{.Names}}' | grep -c uah-beta-backend || true)
    OLLAMA_OK=$(docker exec uah-beta-backend python3 -c "import httpx; httpx.get('http://10.8.0.8:11434/api/tags',timeout=3)" 2>/dev/null && echo "1" || echo "0")
